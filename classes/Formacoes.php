...
// $data['password']=$this->generatePassword(8);
    ...
    static function getFicheiros($data) {
    ...
    static function getFicheiro($data) {
        $query = "SELECT * FROM (SELECT " .
                "d.idDocuments, " .
                "d.idCourse, " .
                "d.idCourses, " .
    ...
    static function restaurarFormacoesFicheiros($data) {
        $con       = new Database ();
        $resultado = $con->set('START TRANSACTION');
    ...
    static function getFormacoesModulos($data) {
    ...
    static function inserirFormacoesInformacao($data) {//vem do upload.php
        $con       = new Database ();
        ...
        $query     = "INSERT INTO courses_informations (idCourses,idCourse,name,observations,status,document,documentBlob,date) VALUES (" .
                $data['idCourses'] . "," . $resultado[0]['idCourse'] . ",'" . (key_exists('name', $data) ? $data['name'] : '') . "','" .
    ...
    static function atualizarFormacoesInformacao($data) {//vem do upload.php
        $con       = new Database ();
        $query     = "SELECT idCourse FROM courses WHERE idCourses=" . $data['idCourses'];
        $resultado = $con->get($query);
        if (!$resultado) {
    ...
    static function getInformacao($data) {
        $query = "SELECT " .
                "idInformations, " .
                "idCourse, " .
                "idCourses, " .
    ...
    static function inserirFormacoesFicheiro($data) {//vem do upload.php
        $con       = new Database ();
        $resultado = $con->set('START TRANSACTION');
    ...
    static function atualizarFormacoesFicheiro($data) {//vem do upload.php
        $con       = new Database ();
        $query     = "SELECT idCourse FROM courses WHERE idCourses=" . $data['idCourses'];
        $resultado = $con->get($query);
        if (!$resultado) {
    ...
    static function inserirFormacoesFicheiros($data) {
    ...
    static function apagarFormacoesFicheiros($data) {
    ...
    static function aprovarFormacoesFicheiros($data) {//<?=$docs[$i]['idCourse']."_".$docs[$i]['idCourses']."_".$docs[$i]['idModules']."_".$docs[$i]['idDocuments']
        ...
        if (($resultado['equipa'] == 0 && $_SESSION['users']->permission == 'Equipa Executiva') ||
                ($data['action'] == 'equipaExecutiva' && $_SESSION['users']->permission == 'Equipa Executiva')) { //Não pertence à equipa de curso, mas é equipa executiva
...
//                    "observations='" . $rowData['observations'] . "' " .
    ...
    static function apagarFormacoesInformacoes($data) {
