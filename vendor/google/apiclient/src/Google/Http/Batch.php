...
class Google_Http_Batch {
    ...
    Google_Client $client, $boundary = false, $rootUrl = null, $batchPath = null
    ) {
        $this->client    = $client;
...
EOF;
        ...
        foreach ($this->requests as $key => $request) {
            $firstLine = sprintf(
                    '%s %s HTTP/%s', $request->getMethod(), $request->getRequestTarget(), $request->getProtocolVersion()
            );
            ...
            $body .= sprintf(
                    $batchHttpTemplate, $this->boundary, $key, $firstLine, $headers, $content ? "\n" . $content : ''
            );
            ...
            foreach ($parts as $i => $part) {
                ...
                if (!empty($part)) {
                    ...
                    $response = new Response(
                            $status, $partHeaders, Psr7\stream_for($partBody)
                    );
