...
class Google_AccessToken_Verify {
    ...
    ClientInterface $http = null, CacheItemPoolInterface $cache = null, $jwt = null
    ) {
        if (null === $http) {
            $http = new Client();
        }
        ...
        foreach ($certs as $cert) {
            ...
            try {
                $payload = $this->jwt->decode(
                        $idToken, $rsa->getPublicKey(), array('RS256')
                );
    ...
    private function retrieveCertsFromLocation($url) {
        ...
        throw new Google_Exception(
        sprintf(
                'Failed to retrieve verification certificates: "%s".', $response->getBody()->getContents()
