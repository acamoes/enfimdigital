        ...
        {foreach $equipaExecutiva->modulos as $modulos}
            <tr>
                ...
                <td class="actions" align="right"><a
                        ...
                        onclick="request('action={$modulo}&task=ver&tab={$currentTab}&idModules={$modulos['idModules']}','form');"></a>
                    <a class="button small icon fa-edit"
                       style="cursor: pointer; padding: 0 0 0 5pt"
                       onclick="request('action={$modulo}&task=editar&tab={$currentTab}&idModules={$modulos['idModules']}','form');"></a>
                    <a class="button small icon fa-eraser"
                       style="cursor: pointer; padding: 0 0 0 5pt"                       
                       onclick="request('action={$modulo}&task=apagar&tab={$currentTab}&idModules={$modulos['idModules']}','{$modulo}Msg');
