                ...
                <td class="actions" align="right">
                    <a  class="button small icon fa-user"
                        ...
                        onclick="
                                ...
                                request('action={$action}&task=novo&tab={$currentTab}&idModules={$sessoes['idModules']}&idCourse={$sessoes['idCourse']}&searchUtilizadores=1', 'smallForm');"></a>
                    {if $sessoes['name'] eq 'DIREÇÃO' || $users->permission eq 'Equipa Executiva'}
                        <a class="button small icon fa-edit"
                           style="cursor: pointer; padding: 0 0 0 5pt"
                           onclick="request('action={$action}&task=editar&tab={$currentTab}&idModules={$sessoes['idModules']}&idCourse={$sessoes['idCourse']}', 'form');"></a>
                        <a class="button small icon fa-eraser"
                           style="cursor: pointer; padding: 0 0 0 5pt"                       
                           onclick="if (confirm('Tem a certeza que pretende apagar o registo?')) {ldelim}
                                       $.when(request('action={$action}&task=apagar&tab={$currentTab}&idModules={$sessoes['idModules']}&idCourse={$sessoes['idCourse']}', '{$action}Msg')).
